...
$sql = "SELECT * FROM tb_butawarna WHERE id_surat = '$id_surat'";
$ambil_data = mysqli_query($koneksi, $sql);		
$hasil = mysqli_fetch_array($ambil_data);
?>

	<h1>Ubah Data Surat Keterangan Tidak Buta Warna</h1>
...
<?php
	...
	if(isset($_POST["edit"])){
		...
		$query = "update tb_butawarna set nama_dokter='$nama_dokter', ttl='$ttl', alamat='$alamat', hasil='$hasil', keperluan='$keperluan', tanggal_pembuatan='$tanggal_pembuatan' where id_surat='$id_surat'";
		$exe = mysqli_query($koneksi, $query);
		if($exe){
			echo "<script>alert('Data Berhasil Di Ubah');window.location='tampil_butawarna.php';</script>";
		}else{
